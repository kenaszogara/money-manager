...
function AddPage(props) {
  ...
  // test data
  const categories = [
    { 'name': 'Food', 'color': 'green', 'type': 'expense' },
    { 'name': 'Car', 'color': 'blue', 'type': 'expense' },
    { 'name': 'Salary', 'color': 'green', 'type': 'income' },
    { 'name': 'Gift', 'color': 'red', 'type': 'income' },
  ]
  ...
  return (
    <div className='m-auto max-w-4xl'>
      ...
      <div className='flex flex-col w-full h-full bg-white rounded shadow-md mb-8 p-4'>
        <select className='bg-gray-200 w-32 h-8 mr-auto' onChange={handleSelect}>
      ...
      {/* Input */}
      {
        ...
        <div className='w-full h-full bg-white rounded shadow-md flex flex-wrap flex-row items-center p-4'>
          {/* categories-btn */}
          <div className={`bg-${activeCategory.color}-300 mr-2 w-16 h-16 shadow-lg rounded-full flex items-center justify-center`}>
            <InfoOutlinedIcon />
          </div>
